...
class InfoServicer(Info_pb2_grpc.InfoServicer):
    ...
    def GetInfo(self, request, context):
        response = Info_pb2.Reply()
                                                                        #customer_id but named productid
        res = Controller.retrieve_shopping_cart_by_customer_id(mongodbconnection,request.productid)
        ...
        response.mapfield["cartitemlist"]=str(res)
