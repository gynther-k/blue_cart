...
class InfoServicer(Info_pb2_grpc.InfoServicer):
    ...
    def GetInfo(self, request, context):
        ...
        res = Controller.retrieve_review_By_Product_Id(mongodbconnection,request.productid)
        ...
        response.mapfield["productid"]=str(request.productid)
        listToStr = ' '.join(map(str, res)) 
        response.mapfield["reviewlist"]='['+listToStr+']'
